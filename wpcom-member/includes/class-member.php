...
class Member {
    ...
    function localize_script( $scripts ){
        ...
        if( $captcha == 'noCaptcha' && isset($options['nc_appkey']) && $options['nc_appkey']!='' && $options['nc_access_id']!=''  && $options['nc_access_secret']!='' ) {
            ...
            $scripts['_Captcha'] = array(
                'title' => __('Security Verification', 'wpcom'),
                'barText' => __('Drag to complete the jigsaw', 'wpcom'),
                'loadingText' => __('Jigsaw is loading', 'wpcom'),
                'failedText' => __('Please try again', 'wpcom'),
            );
    ...
    function account_tabs_bind(){
        ...
        <?php } else if($action=='change'){
            ...
            $steps = array(
                0 => _x('STEP 1', '验证方式', 'wpcom'),
                1 => _x('STEP 2', '安全验证', 'wpcom'),
                2 => _x('STEP 3', '绑定账号', 'wpcom')
            );
            $current_step = 0;
    ...
    function login_form(){
        ...
        <form id="login-form" class="member-form j-member-form" method="post">
            ...
            <div class="member-remember checkbox">
                ...
                <a class="member-form-forgot" href="<?php echo esc_url(wp_lostpassword_url()); ?>"><?php esc_html_e('Forgot password?', 'wpcom');?></a>
            </div>
    ...
    function lostpassword_form_reset(){
        if( isset($GLOBALS['reset-error']) && $GLOBALS['reset-error']){ ?>
            <div class="member-form lostpassword-form lostpassword-form-status">
                ...
                <a class="wpcom-btn btn-primary" href="<?php echo esc_url(wp_lostpassword_url()); ?>"><?php esc_html_e('Click here to resend password reset email', 'wpcom');?></a>
            </div>
        <?php }else{
    ...
    function lostpassword_form_finished(){ ?>
        <div class="member-form lostpassword-form lostpassword-form-status">
            ...
            <a class="wpcom-btn btn-primary" href="<?php echo esc_url(wp_login_url());?>"><?php esc_html_e(' Click here to return to the login page', 'wpcom');?></a>
        </div>
    <?php }
        ...
        if( $args && isset($args['type']) ){
            ...
            switch ($args['type']) {
                ...
                case '_Captcha':
                    ...
                    break;
                case 'smsCode':
                    $html = '<div class="form-group sms-code">'.wp_nonce_field( 'send_sms_code', 'send_sms_code_nonce', true, false ).'<label>'.$icon.' <input type="text" class="form-input is-input require" id="'.$name.'" name="'.$name.'" placeholder="'.$placeholder.'"'.($validate?' data-rule="'.$validate.'"':'').' data-label="'.$label.'" autocomplete="off"></label><div class="wpcom-btn btn-lg send-sms-code j-send-sms-code" data-target="'.(isset($args['target']) ? $args['target'] : '').'">'.__('Get Code', 'wpcom').'</div></div>';
                    break;
                case 'hidden':
        ...
        if( $socials ){ ?>
            <ul class="member-social-list">
                ...
                <li class="social-item social-<?php echo esc_attr($social['name']);?>">
                    ...
                    <a href="<?php echo esc_url(wpcom_social_login_url($social['name']));?>"<?php echo isset($options['social_login_target']) && !$options['social_login_target'] ? '' : ' target="_blank"';?> data-toggle="tooltip" data-placement="top" title="<?php echo esc_attr(sprintf( __('Log in with %s', 'wpcom'), $social['title'] ));?>" aria-label="<?php echo esc_attr($social['title']);?>">
        ...
        if ( is_wpcom_member_page( 'account' ) ) {
            ...
            if($subpage === 'reset'){
                ...
                if( ! $user || is_wp_error( $user ) ){
                    ...
                    if ( $user && $user->get_error_code() === 'expired_key' )
                        $GLOBALS['reset-error'] = __('Your password reset link has expired', 'wpcom');//'您的密码重置链接已过期，请重新请求新链接。';
                    else
                        $GLOBALS['reset-error'] = __('Your password reset link appears to be invalid', 'wpcom');//'您的密码重设链接无效，请重新请求新链接。';
                }
            }
            ...
            if( $get_user->ID ){
                ...
                if( $get_user->user_status=='-1' && $member_reg_active!='0' ){
                    ...
                    if($member_reg_active=='1'){
                        ...
                        $err = sprintf( esc_html__( 'Please activate your account. %1$s Resend activation email %2$s', 'wpcom' ), '<a href="'.$resend_url.'" target="_blank">', '</a>' );
    ...
    function views_users( $views ){
        ...
        $count2 = count($users2);
        $views[ 'blacklist' ] = '<a href="'.admin_url('users.php').'?status=blacklist" ' . $current2 . '>'. __( 'Blacklist', 'wpcom' ) . ' <span class="count">（'.$count2.'）</span></a>';
    ...
    function bulk_actions_users( $actions ){
        if( current_user_can( 'edit_users' ) ) {
            ...
            $actions['blacklist'] = __('Add to blacklist', 'wpcom');
            $actions['remove-blacklist'] = __('Remove from blacklist', 'wpcom');
        }
        return $actions;
    }
    ...
    function user_data_stats($user, $link=true){
        ...
        $this->user_stats_item($posts, _x('posts', 'stats', 'wpcom'), $link ? $posts_url : '');
        if(wpmx_comment_status()) $this->user_stats_item($comments, _x('comments', 'stats', 'wpcom'), $link ? $comments_url : '');
        ...
        if(defined('QAPress_VERSION')){
            ...
            if($questions) $this->user_stats_item($questions, _x('questions', 'stats', 'wpcom'), $link ? $questions_url : '');
            if($answers) $this->user_stats_item($answers, _x('answers', 'stats', 'wpcom'), $link ? $questions_url : '');
        }
        ...
        if((class_exists('\WPCOM_Follow') || class_exists(Follow::class)) && isset($options['member_follow']) && $options['member_follow']=='1'){
            ...
            $this->user_stats_item($followers, _x('followers', 'stats', 'wpcom'), $link ? $followers_url : '');
        }
    }
